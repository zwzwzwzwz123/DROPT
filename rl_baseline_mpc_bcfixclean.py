...
# -*- coding: utf-8 -*-
"""
SAC + expert imitation baseline aligned with main_building_fno_guided_bcfix_clean.py.

Alignment goals:
- use the same BC-fix-clean environment wrapper
- keep expert_action aligned to current state s_t
- use the same reward / violation penalty definition
- keep reward_normalization behavior consistent with bcfixclean
"""
...
class SACImitationPolicy(SACPolicy):
    def __init__(
        ...
        *args,
        ...
        bc_weight_decay_steps: int = 150000,
        **kwargs: Any,
    ) -> None:
    ...
    def learn(self, batch, **kwargs: Any) -> Dict[str, float]:
        ...
        if self._is_auto_alpha:
            result["loss/alpha"] = alpha_loss.item()  # type: ignore[name-defined]
            result["alpha"] = self._alpha.item()  # type: ignore[attr-defined]
        return result


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SAC + expert imitation baseline aligned with the bcfixclean environment."
    )

    parser.add_argument("--building-type", type=str, default=DEFAULT_BUILDING_TYPE)
    parser.add_argument("--weather-type", type=str, default=DEFAULT_WEATHER_TYPE)
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use the full-year trajectory by setting episode_length=None.",
    )
    ...
    parser.add_argument("--add-violation-penalty", dest="add_violation_penalty", action="store_true")
    parser.add_argument("--no-add-violation-penalty", dest="add_violation_penalty", action="store_false")
    parser.set_defaults(add_violation_penalty=True)
    ...
    parser.add_argument("--expert-type", type=str, default="mpc", choices=["mpc", "pid", "rule", "bangbang"])
    ...
    parser.add_argument("--log-prefix", type=str, default="sac_baseline_mpc_bcfixclean")
    parser.add_argument("--device", type=str, default="cuda:0")
    parser.add_argument("--vector-env-type", choices=["dummy", "subproc"], default="dummy")
...
def build_sac_policy(args: argparse.Namespace, env, device: torch.device) -> SACImitationPolicy:
...
def main():
    ...
    if getattr(args, "reward_normalization", False) and getattr(args, "n_step", 1) > 1:
        print("Warning: n_step>1 is incompatible with reward_normalization; disabling it.")
    ...
    print("\n" + "=" * 70)
    print("  SAC Expert-Imitation Baseline Aligned With BCFixClean")
    print("=" * 70)
    ...
    def save_checkpoint_fn(epoch, env_step, gradient_step):
        if args.save_interval > 0 and epoch % args.save_interval == 0:
            torch.save({"model": policy.state_dict()}, os.path.join(log_path, f"checkpoint_{epoch}.pth"))
