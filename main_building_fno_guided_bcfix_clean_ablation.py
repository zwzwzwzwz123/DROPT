...
from diffusion.model_fno import DiffFNO, SpectralConv1d
...
class DiffFNO_NoResidual(nn.Module):
    """DiffFNO variant without the residual linear shortcut and gate."""
    ...
    def __init__(
        ...
        super().__init__()
        act_cls = nn.Mish if activation == "mish" else nn.ReLU
    ...
    def forward(self, x: torch.Tensor, time: torch.Tensor, state: torch.Tensor) -> torch.Tensor:
        ...
        for spec_conv, pw_conv in zip(self.spectral_layers, self.pointwise):
            y = self.activation(spec_conv(y) + pw_conv(y) + cond)

        return self.out_proj(y).squeeze(1)
...
def _parse_fno_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="residual",
        choices=["residual", "nores"],
        help="Backbone variant for ablations.",
    )
...
def get_args():
    ...
    args.backbone_variant = fno_args.backbone_variant
    ...
    guidance_tag = "guided" if float(args.guidance_scale) > 0 else "noguide"
    variant_tag = "nores" if args.backbone_variant == "nores" else "residual"
    if args.log_prefix == "default":
        args.log_prefix = f"diffusion_fno_guided_bcfix_clean_{variant_tag}_{guidance_tag}"
    args.algorithm = f"diffusion_fno_guided_bcfix_clean_{variant_tag}_{guidance_tag}"
    # 覆盖默认运行超参以加速实验
...
def main():
    ...
    # 网络 ---------------------------------------------------------------
    backbone_label = "DiffFNO(NoRes)" if args.backbone_variant == "nores" else "DiffFNO"
    backbone_cls = DiffFNO_NoResidual if args.backbone_variant == "nores" else DiffFNO

    print(f"\nBuilding {backbone_label} backbone ...")
    state_dim = env.state_dim
    action_dim = env.action_dim
    max_action = 1.0

    fno_backbone = backbone_cls(
        state_dim=state_dim,
    ...
    print(f"  backbone_variant={args.backbone_variant}")
    print(f"  {backbone_label} params: {sum(p.numel() for p in fno_backbone.parameters()):,}")
