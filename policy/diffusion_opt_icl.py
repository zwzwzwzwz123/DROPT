...
class DiffusionOPT(BasePolicy):
    ...
    def _update_critic(self, batch: Batch) -> tuple:
        ...
        # 计算MSE损失（两个Q网络的损失之和）
        # 使用 Huber 损失缓和极端 TD 误差，降低梯度爆炸风险
    ...
    def _update_bc(self, batch: Batch, update: bool = False) -> Optional[torch.Tensor]:
        ...
        # 使用 action 掩码忽略填充维度
        # 假设 obs_ 中的末尾包含 state_mask+action_mask，长度 base_state_dim+action_dim
        # action_mask 在末尾 action_dim 部分，填充值为0
        mask = None
        if obs_.shape[1] >= expert_actions.shape[1]:
            mask = obs_[:, -expert_actions.shape[1]:]
            mask = (mask.abs() > 1e-8).float()
        else:
            mask = torch.ones_like(expert_actions, device=self._device)
        mask_sum = mask.sum(dim=1, keepdim=True).clamp(min=1.0)
        pred = self._actor(obs_)
        mse = (pred - expert_actions) ** 2
        bc_loss = ((mse * mask).sum(dim=1, keepdim=True) / mask_sum).mean()
